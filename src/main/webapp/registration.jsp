<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %> 
    
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title></title>
<jsp:include page="resources/common.jsp"></jsp:include>
</head>
<body>
	<div class="container-fluid">
		<jsp:include page="resources/menu.jsp"></jsp:include>
		<div class="container">
	<h3 class="my-4 text-center">All Registrations</h3>
	<div class="btn-gp text-right">
		<c:url var="add" value="/reg-add"></c:url>
		<a href="${add }" class="btn btn-outline-secondary">Add New Registration</a>
	</div>
	<table class="table bg-secondary text-light mt-3">
		<thead>
			<tr>
				<th>No</th>
				<th>Student</th>
				<th>Class</th>
				<th>Registration Date</th>
				<th>Paid Amount</th>
				<th>Action</th>
			</tr>
		</thead>
		<tbody>
			<!-- registration List -->
			<c:forEach items="${registrations}" var="r">
				<tr>
					<td><c:set var="numberOfRows" value="${numberOfRows+1}"/>
						<c:out value="${numberOfRows}"/></td>
					<td>${r.student.name }</td>
					<td>${r.classes.name}</td>
					<td>${r.reg_date }</td>
					<td>${r.paid_amt }</td>
					<td>
						<c:url var="edit" value="/reg-edit">
								<c:param name="regid">${r.id}</c:param>
						</c:url> <a href="${edit}" class="btn btn-outline-success">Edit</a>
						<c:url var="delete" value="/reg-delete">
							<c:param name="regid">${r.id }</c:param>
						</c:url> <a href="${delete}" class="btn btn-outline-danger">Delete</a>
					</td>
				</tr>
			</c:forEach>
			</tbody>
	</table>
	</div>
	</div>
</body>
</html>